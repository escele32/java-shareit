...
public class ItemRequestControllerTests {
    ...
    void testItemRequestCreate() throws Exception {
        when(itemRequestClient.create(anyLong(), any())).thenReturn(ResponseEntity.ok().body(itemRequestDto));
    ...
    void testItemRequestGetById() throws Exception {
        when(itemRequestClient.getById(anyLong(), anyLong())).thenReturn(ResponseEntity.ok().body(itemRequestDto));
        mockMvc.perform(get(ApiPath.REQUESTS + ApiPath.REQUESTID, requestDto.getOwnerId(),
                ...
                .andExpect(jsonPath("$.items[0].ownerId", is(2)));
        verify(itemRequestClient).getById(anyLong(), anyLong());
    }

    @Test
    void testItemRequestGetOwnerRequests() throws Exception {
        when(itemRequestClient.getOwnerRequests(anyLong()))
                .thenReturn(ResponseEntity.ok().body(List.of(itemRequestDto)));
        ...
        verify(itemRequestClient).getOwnerRequests(anyLong());
    }
